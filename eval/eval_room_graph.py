...
def gate(
    n=30, seed0=230000, n_rooms_list=(3, 4), speed=0.6, clutter=0, clutter_lane=0.0
):
    ...
    for nr in n_rooms_list:
        for i in range(n):
            sc = n_room(
                seed0 + i, n_rooms=nr, clutter=clutter, clutter_lane=clutter_lane
            )
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.0,
        help="keep |y| < this clear of boxes (a traversal lane)",
    )
    ...
    r = gate(
        ...
        clutter_lane=args.clutter_lane,
    )
    b, c = r["beacon_room_acc"], r["count_acc"]
    print(
        f"\n[room-graph] clutter={args.clutter} lane={args.clutter_lane} "
        f"found {r['found']} | "
